...
smooth.spline(test$data[[1]]$D, test$data[[1]]$age)
...
full_dt_mort %>%
  ...
  filter(status == "mx") %>% 
  ggplot() +
  geom_line(aes(x = age, y = mx, color = sex)) +
...
mrt <- full_dt_mort %>% 
  ...
  pivot_wider(names_from = status,
              values_from = val) %>% 
  full_join(
    dplyr::select(full_dt_prev, -val)
  ) %>% 
  dplyr::select(sex, cause, year, age, mx_u, mx_h, mx, prev = prv)
